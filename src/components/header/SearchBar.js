    ...
    if(nameInput !== "") {
      (async () => {
        try {
          ...
          const uniqueCards = indexOfUniqueCards.map(index => firstXCards[index])
          if (controller) {
            setMatchedCards(uniqueCards)
          } // se crea el array definitivo seleccionando las cartas con los índice del array anterior
        }
        catch(err){
          console.log(err)
        }
